...
class AntiCheatingSystem {
    ...
    preventTabSwitching() {
        document.addEventListener('visibilitychange', () => {
            if (document.hidden) {
                ...
                this.warnUser('Quitter cette page pendant l\'examen est considéré comme une tentative de triche');
    ...
    preventPrintScreen() {
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'PrintScreen') {
                this.warnUser('Les captures d\'écran ne sont pas autorisées pendant l\'examen');
            ...
            if (e.key === 'F12') {
                this.warnUser('L\'utilisation des outils de développeur n\'est pas autorisée');
    ...
    enableFullscreen() {
        ...
        document.addEventListener('fullscreenchange', () => {
            ...
            if (!this.fullscreenActive && this.config.fullscreenMode) {
                this.warnUser('Quitter le mode plein écran n\'est pas recommandé pendant l\'examen');
    ...
    checkIdleTime() {
        ...
        document.addEventListener('mousemove', () => this.lastActive = new Date());
    ...
    checkWindowSize() {
        ...
        if (window.innerWidth < 800 || window.innerHeight < 600) {
            this.warnUser('Redimensionner la fenêtre à une taille trop petite n\'est pas autorisé');
            this.logActivity('suspicious_resize', { width: window.innerWidth, height: window.innerHeight });
        }
    }
    ...
    shuffleOptions(questionId) {
        // Implementation depends on your DOM structure
        const optionsContainer = document.querySelector(`#question-${questionId} .mcq-options`);
    ...
    warnUser(message) {
        ...
        if (this.warningCount >= this.config.maxWarnings) {
            ...
            alert('Trop d\'activités suspectes détectées. Votre examen va être soumis automatiquement.');
